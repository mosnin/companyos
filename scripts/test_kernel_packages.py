...
spec = importlib.util.spec_from_file_location(
    "kernels",
    core / "skills/company-os/elastic-company-os/scripts/kernel_manager.py",
)
...
with tempfile.TemporaryDirectory() as temporary:
    ...
    for repo in ("business-OS", "design-os", "product-os"):
        ...
        assert not any(
            ...
            or p.startswith(".env")
            for p in paths
        ), "Private or generated cache file in package"
        ...
        if repo == "business-OS":
            ...
            assert len(business_skills) >= 732
        else:
            if repo == "design-os":
                assert ".agents/skills/design-os/SKILL.md" in paths
                assert len([p for p in paths if p.endswith("SKILL.md")]) == 32
            else:
                assert "skills/product-os/SKILL.md" in paths
                assert len([p for p in paths if p.endswith("SKILL.md")]) == 85
                assert not any(
                    p.startswith(prefix)
                    for p in paths
                    for prefix in ("scripts/", "tests/", "evals/", "research/")
                ), "Product OS package includes development-only files"
        ident = package["name"].split("/")[1]
        registry.releases[ident] = {
            package["version"]: {
                "archive": stage / package["filename"],
                "dist": {"integrity": package["integrity"]},
            }
        }
        print(
            ident,
    ...
    state = manager.install(
        ...
        binding={"companyId": "fixture-company", "revision": 1},
    )
    manager.verify(state)
    assert set(state["installed"]) == {"business-os", "design-os", "product-os"}
    assert state["desiredRevision"] == 1
    manager.initialize()
    print(
        "PASS: real npm archives installed, verified and initialized; "
