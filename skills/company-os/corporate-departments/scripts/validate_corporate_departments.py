#!/usr/bin/env python3
"""Validate the Corporate Departments pack and spawn template.

The template is a thinking overlay for named Company OS departments. It cannot
be spawned as the master persona, a Luna worker, or a fourth executable hop.
"""
...
SCHEMA = "company-os.corporate-departments-spawn-template.v1"
TEMPLATE_ID = "corporate-departments"
REQUIRED_SKILLS = ("manage-company-program", "corporate-departments")
...
SOURCE_FILES = (
    ...
    "05-finance.txt",
    "06-people.txt",
    "07-strategy-product.txt",
    "08-preset-departments.txt",
)
TOP_LEVEL = {
    "schema",
    "template_id",
...
SKILL_MARKERS = (
    "how company os departments think",
    "do not send this skill to luna workers",
    "do not use it as the master persona",
    "does not own",
    "named department function",
    "not a claim of personal identity",
    "marketing creates demand",
    "sales converts qualified demand",
    "do not merge their decision rights",
    "issue trees before workplans",
...
def validate_pack(root: Path = ROOT) -> list[str]:
    ...
    if not text.startswith("---\nname: corporate-departments\n"):
        errors.append("SKILL.md name must be corporate-departments")
    ...
    for host in ("openai.yaml", "grok.yaml", "claude.yaml"):
        ...
        if "$corporate-departments" not in host_text:
...
def validate_spawn_template(payload: Any) -> list[str]:
    ...
    if not isinstance(skills, list) or list(skills) != list(REQUIRED_SKILLS):
        errors.append(
            "spawn skills must be the manager role skill plus corporate-departments"
        )
    ...
    if not isinstance(use_when, list) or list(use_when) != list(USE_WHEN):
        errors.append("use_when must be the corporate-departments lanes")
    return errors
