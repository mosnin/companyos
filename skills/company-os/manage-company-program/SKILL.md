...
2. Before design or dispatch, use `$assign-capability-skills` to select the
   smallest audited bundle, including zero. Verify catalog, request,
   entrypoint, and assignment digests. Never load bodies during discovery,
   widen authority, or hand-edit bindings.
3. Run the Program Preflight Compiler in
   ...
   Compile and `verify` with the same sources before dispatch. Bind skills only
   to their exact packet; retain digests and dispatch by packet reference.
...
6. Before each worker launch, use `$force-first-execution` to create one
   ...
   At design time, freeze required versus optional release scope and the
...
Preflight is a dispatch gate. Semantics owns terms, constants, authority,
prohibitions, evidence, oracles, and capabilities; the host supplies runtimes
and locators; definitions bind cited evidence, nonempty oracles, and disjoint
project-relative writer scopes. Managers and Luna tasks receive only their
compiled references and slices. Rerun `verify` after transport; mutation,
extras, stale parents, or unbound digests stop.
