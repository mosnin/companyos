...
def validate_blueprint(value: dict[str, Any]) -> None:
    ...
    if not isinstance(identity, dict):
        raise BlueprintError("blueprint.identity must be an object")
    ...
    if not isinstance(market, dict):
        raise BlueprintError("blueprint.market must be an object")
    ...
    operating = value.get("operating_model")
    if not isinstance(operating, dict):
        raise BlueprintError("blueprint.operating_model must be an object")
    require_string_list(operating.get("archetypes"), "operating_model.archetypes", nonempty=True)
    ...
    for index, override in enumerate(overrides):
        ...
        require_id(override.get("department_id"), f"department override {index}.department_id")
    ...
    if not isinstance(authority, dict):
        raise BlueprintError("blueprint.authority must be an object")
    ...
    if not isinstance(brand, dict):
        raise BlueprintError("blueprint.brand must be an object")
    ...
    if not isinstance(knowledge, dict):
        raise BlueprintError("blueprint.knowledge must be an object")
    ...
    for index, asset in enumerate(assets):
        if not isinstance(asset, dict):
            raise BlueprintError(f"asset {index} must be an object")
        ...
        if content_sha256 is not None and re.fullmatch(r"[0-9a-f]{64}", str(content_sha256)) is None:
            raise BlueprintError(f"asset {asset_id}.content_sha256 is invalid")
    ...
    for index, integration in enumerate(integrations):
        if not isinstance(integration, dict):
            raise BlueprintError(f"integration {index} must be an object")
        ...
        if reference is not None and re.fullmatch(r"[A-Z][A-Z0-9_]*", str(reference)) is None:
    ...
    if not isinstance(storage, dict) or set(storage) != {"adapter", "dsn_env", "schema"}:
        raise BlueprintError("blueprint.storage must define adapter, dsn_env, and schema")
    ...
    if re.fullmatch(r"[A-Z][A-Z0-9_]*", dsn_env) is None:
    ...
    for index, item in enumerate(unknowns):
        ...
        require_id(item.get("id"), f"unknown {index}.id")
...
def compile_artifacts(
    ...
    knowledge_graph = {
        ...
        "classifications": blueprint["knowledge"]["classifications"],
        "company_id": company_id,
    ...
    integration_registry = {
        ...
        "integrations": sorted(integrations, key=lambda item: item.get("id", "")),
    }
    storage_plan = {
...
def verify_compiled(output: Path) -> dict[str, Any]:
    manifest = read_json(output / "manifest.json", "compiled manifest")
    if manifest.get("$schema") != "company-os.compiled-company-manifest.v1":
        raise BlueprintError("compiled manifest schema is invalid")
    expected_names = {item.get("path") for item in manifest.get("files", []) if isinstance(item, dict)} | {"manifest.json"}
    actual_names = {path.name for path in output.iterdir() if path.is_file() and not path.is_symlink()}
