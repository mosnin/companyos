...
def _validate_calibrations(
    ...
    for index, raw in enumerate(_array(value, "calibration receipts")):
        ...
        receipt_digest = _verify_self_digest(
            receipt,
            "receipt_sha256",
            f"calibration[{index}]",
        )
        evaluator_id = _text(receipt.get("evaluator_id"), f"calibration[{index}].evaluator_id")
        if evaluator_id in seen:
            raise OutcomeControlError("E_DUPLICATE", f"duplicate calibration for {evaluator_id}")
        seen.add(evaluator_id)
        if receipt.get("passed") is not True:
            raise OutcomeControlError("E_CALIBRATION", f"calibration failed for {evaluator_id}")
        receipts.append(receipt)
        bindings.append({"evaluator_id": evaluator_id, "receipt_sha256": receipt_digest})
    receipts.sort(key=lambda item: str(item.get("evaluator_id", "")))
    bindings.sort(key=lambda item: item["evaluator_id"])
