#!/usr/bin/env python3
"""Validate the Steve product-doctrine pack and spawn template.

The template is a thinking overlay for product, brand, UX, and customer Astra
managers. It cannot be spawned as the master persona, a Luna worker, or a
default for every manager lane.
"""
...
SCHEMA = "company-os.steve-spawn-template.v1"
TEMPLATE_ID = "steve"
REQUIRED_SKILLS = ("manage-company-program", "steve")
...
SOURCE_FILES = (
    "00-index.txt",
    "01-charter-standards.txt",
    "02-customer-backwards-method.txt",
    "03-stick-figure-thought-bubble.txt",
    "04-via-negativa-simplification.txt",
    "05-focus-signal-noise-quadrant.txt",
    "06-synthesis-humanities-science.txt",
    "07-design-for-intuition-defaults-flow.txt",
    "08-craft-details-coherence.txt",
    "09-product-ecosystem-strategy.txt",
    "10-storytelling-frameworks-scqa-rule3.txt",
    "11-a-players-dri-accountability.txt",
    "12-rdf-goals-weekly-operating-system.txt",
    "product-demo-framework.md",
)
TOP_LEVEL = {
    "schema",
    "template_id",
...
def validate_pack(root: Path = ROOT) -> list[str]:
    ...
    if not text.startswith("---\nname: steve\n"):
        errors.append("SKILL.md name must be steve")
    ...
    for phrase in IDENTITY_FORBIDDEN:
        if phrase in folded:
            errors.append("SKILL.md must not claim personal identity")
    if "TODO" in text:
...
def validate_spawn_template(payload: Any) -> list[str]:
    ...
    if not isinstance(skills, list) or list(skills) != list(REQUIRED_SKILLS):
        errors.append("spawn skills must be the manager role skill plus steve")
    ...
    if not isinstance(use_when, list) or list(use_when) != list(USE_WHEN):
        errors.append("use_when must be the product, brand, UX, and customer lanes")
    return errors
