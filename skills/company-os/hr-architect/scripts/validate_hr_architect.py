#!/usr/bin/env python3
"""Validate the HR Architect pack and spawn template.

The template is a thinking overlay for HR and hiring Astra managers. It cannot
be spawned as the master persona, a Luna worker, or a default for every
manager.
"""
...
SCHEMA = "company-os.hr-architect-spawn-template.v1"
TEMPLATE_ID = "hr-architect"
REQUIRED_SKILLS = ("manage-company-program", "hr-architect")
...
SOURCE_FILES = (
    "00-index.txt",
    "01-hr-operating-model.txt",
    "02-org-design-roles.txt",
    "03-recruiting-sourcing.txt",
    "04-selection-interviews-assessments.txt",
    "05-comp-offers-classification.txt",
    "06-onboarding-enablement.txt",
    "07-performance-management.txt",
    "08-culture-retention-offboarding.txt",
    "09-hr-ops-compliance.txt",
    "10-ai-agents-hr-tech-analytics.txt",
)
TOP_LEVEL = {
    "schema",
    "template_id",
...
SKILL_MARKERS = (
    "hr and hiring astra managers",
    "do not send this skill to luna workers",
    "do not use it as the master persona",
    "does not own",
    "scorecards prevent vibes hiring",
    "not legal advice",
    "candidate contact without approval",
)
...
def validate_pack(root: Path = ROOT) -> list[str]:
    ...
    if not text.startswith("---\nname: hr-architect\n"):
        errors.append("SKILL.md name must be hr-architect")
    ...
    for host in ("openai.yaml", "grok.yaml"):
        ...
        if "$hr-architect" not in host_text:
...
def validate_spawn_template(payload: Any) -> list[str]:
    ...
    if not isinstance(skills, list) or list(skills) != list(REQUIRED_SKILLS):
        errors.append("spawn skills must be the manager role skill plus hr-architect")
    ...
    if not isinstance(use_when, list) or list(use_when) != list(USE_WHEN):
        errors.append("use_when must be the people-system lanes")
    return errors
