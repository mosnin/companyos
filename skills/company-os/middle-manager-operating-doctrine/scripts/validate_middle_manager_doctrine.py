...
SCHEMA = "company-os.middle-manager-spawn-template.v1"
TEMPLATE_ID = "middle-manager-operating-doctrine"
REQUIRED_SKILLS = ("manage-company-program", "middle-manager-operating-doctrine")
FORBIDDEN_ROLES = ("master", "worker")
SOURCE_FILES = (
    "00-index.txt",
    "01-charter-ground-rules.txt",
    "02-first-principles-engine.txt",
    "03-systems-engineering-feedback-loops.txt",
    "04-bottleneck-theory-constraint-playbook.txt",
    "05-five-step-algorithm-execution.txt",
    "06-physics-based-cost-curves.txt",
    "07-expected-value-risk-asymmetry.txt",
    "08-tree-of-knowledge-learning-system.txt",
    "09-time-compression-execution-cadence.txt",
    "10-signal-over-noise-talent-density.txt",
    "11-vertical-integration-optionality.txt",
    "12-daily-operating-system-mentor-loop.txt",
...
def validate_pack(root: Path = ROOT) -> list[str]:
    ...
    if not text.startswith("---\nname: middle-manager-operating-doctrine\n"):
        errors.append("SKILL.md name must be middle-manager-operating-doctrine")
    ...
    for host in ("openai.yaml", "grok.yaml"):
        ...
        if "$middle-manager-operating-doctrine" not in host_text:
...
def validate_spawn_template(payload: Any) -> list[str]:
    ...
    if not isinstance(skills, list) or list(skills) != list(REQUIRED_SKILLS):
        errors.append("spawn skills must be the manager role skill plus this doctrine")
