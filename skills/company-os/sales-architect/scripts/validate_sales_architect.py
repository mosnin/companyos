#!/usr/bin/env python3
"""Validate the Sales Architect pack and spawn template.

The template is a thinking overlay for sales Astra managers. It cannot be
...
SCHEMA = "company-os.sales-architect-spawn-template.v1"
TEMPLATE_ID = "sales-architect"
REQUIRED_SKILLS = ("manage-company-program", "sales-architect")
...
SOURCE_FILES = (
    "00-index.txt",
    "01-sales-fundamentals-pipeline.txt",
    "02-prospecting-icp-lists.txt",
    "03-outreach-sequences-multichannel.txt",
    "04-appointments-qualification.txt",
    "05-discovery-needs-roi.txt",
    "06-pitch-demo-presentation.txt",
    "07-objections-negotiation.txt",
    "08-closing-contracting.txt",
    "09-referrals-expansion-accounts.txt",
    "10-sales-ops-management.txt",
)
TOP_LEVEL = {
    "schema",
    "template_id",
...
def validate_pack(root: Path = ROOT) -> list[str]:
    ...
    if not text.startswith("---\nname: sales-architect\n"):
        errors.append("SKILL.md name must be sales-architect")
    ...
    for host in ("openai.yaml", "grok.yaml"):
        ...
        if "$sales-architect" not in host_text:
...
def validate_spawn_template(payload: Any) -> list[str]:
    ...
    if not isinstance(skills, list) or list(skills) != list(REQUIRED_SKILLS):
        errors.append("spawn skills must be the manager role skill plus sales-architect")
    ...
    if not isinstance(use_when, list) or list(use_when) != list(USE_WHEN):
        errors.append("use_when must be the sales-system lanes")
    return errors
