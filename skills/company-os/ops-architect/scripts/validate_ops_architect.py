#!/usr/bin/env python3
"""Validate the Ops Architect pack and spawn template.

The template is a thinking overlay for operations Astra managers. It cannot be
...
SCHEMA = "company-os.ops-architect-spawn-template.v1"
TEMPLATE_ID = "ops-architect"
REQUIRED_SKILLS = ("manage-company-program", "ops-architect")
...
SOURCE_FILES = (
    "00-index.txt",
    "01-operations-fundamentals-flow-metrics.txt",
    "02-capacity-bottlenecks-constraints.txt",
    "03-queueing-systems-littles-law.txt",
    "04-process-structures-layouts.txt",
    "05-visual-tools-networks-scheduling.txt",
    "06-inventory-economics-costs-benefits.txt",
    "07-inventory-management-methods.txt",
    "08-forecasting-demand-planning.txt",
    "09-supply-chain-management.txt",
    "10-project-and-agile-delivery.txt",
)
TOP_LEVEL = {
    "schema",
    "template_id",
...
def validate_pack(root: Path = ROOT) -> list[str]:
    ...
    if not text.startswith("---\nname: ops-architect\n"):
        errors.append("SKILL.md name must be ops-architect")
    ...
    for host in ("openai.yaml", "grok.yaml"):
        ...
        if "$ops-architect" not in host_text:
...
def validate_spawn_template(payload: Any) -> list[str]:
    ...
    if not isinstance(skills, list) or list(skills) != list(REQUIRED_SKILLS):
        errors.append("spawn skills must be the manager role skill plus ops-architect")
    ...
    if not isinstance(use_when, list) or list(use_when) != list(USE_WHEN):
        errors.append("use_when must be the operations-system lanes")
    return errors
