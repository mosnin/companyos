#!/usr/bin/env python3
"""Validate the Business Architect pack and spawn template.

The template is a thinking overlay for business-architecture Astra managers. It
...
SCHEMA = "company-os.business-architect-spawn-template.v1"
TEMPLATE_ID = "business-architect"
REQUIRED_SKILLS = ("manage-company-program", "business-architect")
...
SOURCE_FILES = (
    "00-index.txt",
    "business-model-canvas.txt",
    "core-context-framework.txt",
    "decision-tree-journal.txt",
    "flywheel-model.txt",
    "four-actions-framework.txt",
    "jobs-to-be-done.txt",
    "lean-canvas.txt",
    "mckinsey-7s.txt",
    "north-star-metric.txt",
    "pestel-analysis.txt",
    "porters-five-forces.txt",
    "promise-model.txt",
    "risk-register.txt",
    "strategy-canvas.txt",
    "three-horizon-model.txt",
    "value-builder-system.txt",
    "value-proposition-canvas.txt",
)
TOP_LEVEL = {
    "schema",
    "template_id",
...
def validate_pack(root: Path = ROOT) -> list[str]:
    ...
    if not text.startswith("---\nname: business-architect\n"):
        errors.append("SKILL.md name must be business-architect")
    ...
    for host in ("openai.yaml", "grok.yaml"):
        ...
        if "$business-architect" not in host_text:
...
def validate_spawn_template(payload: Any) -> list[str]:
    ...
    if not isinstance(skills, list) or list(skills) != list(REQUIRED_SKILLS):
        errors.append(
            "spawn skills must be the manager role skill plus business-architect"
        )
    ...
    if not isinstance(use_when, list) or list(use_when) != list(USE_WHEN):
        errors.append("use_when must be the business-architecture lanes")
    return errors
