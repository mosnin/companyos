...
def unsafe_hits(text: str) -> list[str]:
    ...
    for pattern in UNSAFE_PATTERNS:
        ...
        if index < 0:
            continue
        context = lowered[max(0, index - 80):index + len(pattern) + 80]
        if any(marker in context for marker in ("do not", "never", "forbid", "reject", "without")):
            continue
        hits.append(pattern)
    return hits
