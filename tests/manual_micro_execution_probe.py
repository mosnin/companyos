...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="company-os-micro-") as temp:
        ...
        worker_classes = [
            worker.get("work_class")
            for manager in fabric.get("managers", [])
...
""")
        ...
        node_probe = run(
            "node", "-e",
            "const {calculateTip}=require('./product/calculator.js'); const r=calculateTip(100,20,3); if(Math.abs(r.tip-20)>1e-9||Math.abs(r.total-120)>1e-9||Math.abs(r.perPerson-40)>1e-9) process.exit(2); console.log(JSON.stringify(r));",
            cwd=project,
        )
        ...
        advance = run(
            "python3", str(DIRECTOR), "advance",
            "--project-root", str(project),
            "--objective-id", OBJECTIVE_ID,
        )
        status = run(
            "python3", str(DIRECTOR), "status",
            "--project-root", str(project),
            "--objective-id", OBJECTIVE_ID,
        )
        ...
        checks = {
            "exact_company_os_commit": run("git", "rev-parse", "HEAD", cwd=REPO).stdout.strip(),
        ...
        required = [
            checks["start_stage_is_discovery"],
            checks["implementation_dispatched_during_discovery"],
            checks["research_dispatched_during_discovery"],
            checks["reality_spike_instruction_present"],
            checks["product_artifact_created"],
            checks["real_runtime_executed"],
            checks["connected_journey_observed"],
            checks["documentation_spiral_absent"],
        ]
        report = {
            "objective": OBJECTIVE,
            "passed": all(required),
            "checks": checks,
            "start_output": start_payload,
            "advance_output": json.loads(advance.stdout.strip().splitlines()[-1]),
            "status_output": status_payload,
        }
