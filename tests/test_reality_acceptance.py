...
class RealityAcceptanceTests(unittest.TestCase):
 ...
 def _write_sources(self):
  ...
  self._write_json("artifacts.json",self._contract(M.ARTIFACT_SCHEMA,{"ready":True,"artifact_classes":[]})); self._write_json("evaluators.json",self._contract(M.EVALUATOR_SCHEMA,{"ready":True,"evaluators":[]})); self._write_json("benchmarks.json",self._contract(M.BENCHMARK_SCHEMA,{"ready":True,"benchmarks":[]}))
